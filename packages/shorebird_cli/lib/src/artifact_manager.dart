...
class ArtifactManager {
  ...
  Future<FileDownload> startFileDownload(Uri uri, {String? outputPath}) async {
    // flutterpatch: local release cache uses file:// URLs from CodePushClient.
    if (uri.scheme == 'file') {
      final source = File(uri.toFilePath());
      if (!source.existsSync()) {
        throw Exception('Local artifact not found: ${uri.toFilePath()}');
      }
      final File outFile;
      if (outputPath != null) {
        ...
        final tempDir = await Directory.systemTemp.createTemp();
        outFile = await source.copy(p.join(tempDir.path, 'artifact'));
      }
      return FileDownload(
        file: Future.value(outFile),
  ...
  /// Returns the shorebird release supplement directory for the given
  /// [platformSubdir] (e.g. 'ios', 'macos', 'android').
  ///
  /// If [create] is true, the directory is created if it doesn't exist.
  /// Returns null if the directory doesn't exist and [create] is false.
  Directory? getReleaseSupplementDirectory({
    ...
    if (!releaseSupplementDir.existsSync()) {
      if (create) {
        releaseSupplementDir.createSync(recursive: true);
      } else {
        logger.detail(
          'No release supplements found at ${releaseSupplementDir.path}',
        );
        return null;
      }
    }
  ...
  Directory? getIosReleaseSupplementDirectory() {
    return getReleaseSupplementDirectory(platformSubdir: 'ios');
  }
  ...
  Directory? getMacosReleaseSupplementDirectory() {
    return getReleaseSupplementDirectory(platformSubdir: 'macos');
  }

  /// Name of the App.xcframework generated by `flutterpatch release ios-framework`
  static const String appXcframeworkName = 'App.xcframework';

  /// Returns the path to the App.xcframework generated by
  /// `flutterpatch release ios-framework` or
  /// `flutterpatch patch ios-framework`.
